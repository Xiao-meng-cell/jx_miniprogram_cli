// pages/tab_index/search_page/search_page.js
...
Page({
  ...
  jumpSearchResults:function(){
    ...
    wx.navigateTo({
      url: '../search_results/search_results?keyword=' + this.data.searchText,
    })
  },
  ...
  */
  ...
  saveSearchText:function(value){
    ...
    if (flag){
      ...
      wx.setStorageSync('historyLabel_good',this.data.historyLabel)
    }
    }
  },
  ...
  getHistoryLabel:function(){
    var history_label = wx.getStorageSync('historyLabel_good');
    if (!history_label){
      this.setData({
        historyLabel: []
      });
  ...
  clearHistory:function(){
    wx.showModal({
      ...
      success:res=> {
        if (res.confirm) {
          try {
            wx.removeStorageSync('historyLabel_good');
            this.getHistoryLabel();
          } catch (e) {
            wx.showToast({
              title: '清除失败',
